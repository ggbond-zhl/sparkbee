...
第一阶段后端先实现 chargingPoint 与 connector 两套管理 API：chargingPoint 可以先创建基本配置，connector 通过 `/chargingPoints/:chargingPointId/connectors` 单独增删查改。`chargingPoints` 保存 UUID 主键、identity、protocol、centralSystemUrl、厂商型号和可选固件/序列号；`connectors` 保存 UUID 主键、chargingPointId、evseId、connectorId、type、format、powerType、额定电压/电流/功率和 sortOrder。运行态、可用性、协议事件、协议报文和交易历史不进入第一阶段 CRUD 表。
