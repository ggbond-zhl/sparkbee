...
export class ChargingPointEventStreamHub {
  ...
  private projectActorEvent(event: ChargingPointActorEvent): void {
    const projection = this.projections.get(event.chargingPointId) ??
      createEmptyProjection();

    const nextProjection = reduceRuntimeProjection(projection, event);
    this.projections.set(event.chargingPointId, nextProjection);
  }

  private clearProjection(chargingPointId: string): void {
